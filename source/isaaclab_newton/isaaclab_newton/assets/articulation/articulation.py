...
class Articulation(BaseArticulation):
    ...
    def _initialize_impl(self):
        ...
        if self.cfg.articulation_root_prim_path is not None:
            ...
            first_env_root_prim_path = first_env_matching_prim_path + self.cfg.articulation_root_prim_path
        ...
        self._first_env_articulation_root_prim_path = first_env_root_prim_path
        ...
        self._repair_joint_limits_from_usd_if_needed()
        self._process_cfg()
    ...
    def _read_joint_limits_from_usd(self) -> dict[str, tuple[float, float]]:
        """Read explicit joint limits from USD articulation joints and return radians/meters in joint-name order."""
        stage = sim_utils.get_current_stage()
        if stage is None:
            return {}

        root_prim_path = getattr(self, "_first_env_articulation_root_prim_path", None)
        if not isinstance(root_prim_path, str) or len(root_prim_path) == 0:
            return {}

        try:
            joint_prims = sim_utils.get_all_matching_child_prims(
                root_prim_path,
                predicate=lambda prim: prim.IsA(UsdPhysics.RevoluteJoint) or prim.IsA(UsdPhysics.PrismaticJoint),
                stage=stage,
            )
        except ValueError:
            return {}
        ...
        for joint_prim in joint_prims:
            ...
            if joint_prim.IsA(UsdPhysics.RevoluteJoint):
                ...
                lower_raw = joint_api.GetLowerLimitAttr().Get()
                upper_raw = joint_api.GetUpperLimitAttr().Get()
                if lower_raw is None or upper_raw is None:
                    continue
                lower = float(lower_raw) * math.pi / 180.0
                upper = float(upper_raw) * math.pi / 180.0
    ...
    def _repair_joint_limits_from_usd_if_needed(self) -> None:
        """Repair degenerate Newton joint limits using USD-authored joint limits."""
        if self.num_joints == 0:
            return

        invalid_eps = 1.0e-6
        lower_limits = wp.to_torch(self._data.joint_pos_limits_lower).to(self.device).clone()
        upper_limits = wp.to_torch(self._data.joint_pos_limits_upper).to(self.device).clone()

        invalid = (~torch.isfinite(lower_limits)) | (~torch.isfinite(upper_limits))
        invalid |= (upper_limits - lower_limits).abs() <= invalid_eps
        if not bool(invalid.any()):
            return

        limits_from_usd = self._read_joint_limits_from_usd()
        if len(limits_from_usd) == 0:
            return

        repaired_joint_names: list[str] = []
        for joint_idx, joint_name in enumerate(self.joint_names):
            if joint_name not in limits_from_usd:
                continue
            invalid_joint_mask = invalid[:, joint_idx]
            if not bool(invalid_joint_mask.any()):
                continue
            lower, upper = limits_from_usd[joint_name]
            lower_limits[invalid_joint_mask, joint_idx] = lower
            upper_limits[invalid_joint_mask, joint_idx] = upper
            repaired_joint_names.append(joint_name)

        if len(repaired_joint_names) == 0:
            return

        self.write_joint_position_limit_to_sim(lower_limits, upper_limits)

        updated_invalid = (~torch.isfinite(lower_limits)) | (~torch.isfinite(upper_limits))
        updated_invalid |= (upper_limits - lower_limits).abs() <= invalid_eps
        unresolved_joint_ids = torch.nonzero(updated_invalid.any(dim=0), as_tuple=False).squeeze(-1).tolist()
        unresolved_joint_names = [self.joint_names[joint_idx] for joint_idx in unresolved_joint_ids]

        logger.warning(
            "Detected degenerate Newton joint limits; repaired %d joints from USD limits. unresolved=%s",
            len(repaired_joint_names),
