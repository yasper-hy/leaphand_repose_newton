...
class TestJointLimitUsdRepair:
    """Regression tests for Newton joint-limit repair path."""

    @pytest.mark.parametrize(
        ("joint_names", "usd_limits"),
        [
            (
                ["allegro_joint_0", "allegro_joint_1", "allegro_joint_2", "allegro_joint_3"],
                {
                    "allegro_joint_0": (-0.35, 1.10),
                    "allegro_joint_1": (-0.60, 1.25),
                    "allegro_joint_2": (-0.45, 1.35),
                    "allegro_joint_3": (-0.30, 1.20),
                },
            ),
            (
                ["a_0", "a_1", "a_2", "a_3"],
    ...
    @pytest.mark.parametrize("device", ["cpu", "cuda:0"])
    def test_repair_joint_limits_from_usd_updates_degenerate_limits(self, joint_names, usd_limits, device: str):
        articulation, _, _ = create_test_articulation(
            num_instances=2,
        ...
        articulation._first_env_articulation_root_prim_path = "/World/envs/env_0/Robot"
        articulation._read_joint_limits_from_usd = MagicMock(return_value=usd_limits)

        articulation._repair_joint_limits_from_usd_if_needed()

        lower = wp.to_torch(articulation.data.joint_pos_limits_lower)
        upper = wp.to_torch(articulation.data.joint_pos_limits_upper)
        for idx, joint_name in enumerate(joint_names):
            expected_lower, expected_upper = usd_limits[joint_name]
            torch.testing.assert_close(lower[:, idx], torch.full((2,), expected_lower, device=device), atol=1e-6, rtol=0.0)
            torch.testing.assert_close(upper[:, idx], torch.full((2,), expected_upper, device=device), atol=1e-6, rtol=0.0)

        # Regression check for observation unscale denominator safety.
        assert bool(torch.all((upper - lower).abs() > 1.0e-6))

    @pytest.mark.parametrize("device", ["cpu", "cuda:0"])
    def test_repair_joint_limits_from_usd_skips_when_limits_valid(self, device: str):
        articulation, _, _ = create_test_articulation(
            num_instances=2,
            num_joints=4,
            joint_names=["a_0", "a_1", "a_2", "a_3"],
            device=device,
        )
        valid_lower = torch.tensor([[-1.0, -0.5, -0.25, -0.1], [-1.0, -0.5, -0.25, -0.1]], device=device)
        valid_upper = torch.tensor([[1.0, 0.5, 0.25, 0.8], [1.0, 0.5, 0.25, 0.8]], device=device)
        articulation.write_joint_position_limit_to_sim(valid_lower, valid_upper)
        articulation._read_joint_limits_from_usd = MagicMock(side_effect=AssertionError("USD fallback should not run"))

        articulation._repair_joint_limits_from_usd_if_needed()

        lower = wp.to_torch(articulation.data.joint_pos_limits_lower)
        upper = wp.to_torch(articulation.data.joint_pos_limits_upper)
        torch.testing.assert_close(lower, valid_lower, atol=1e-6, rtol=0.0)
        torch.testing.assert_close(upper, valid_upper, atol=1e-6, rtol=0.0)
